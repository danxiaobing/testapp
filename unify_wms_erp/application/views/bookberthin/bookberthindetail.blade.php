<div class="bjui-pageContent">
    <form id="bookberthin-detail-form" action="" class="datagrid-edit-form" data-toggle="validate" data-data-type="json">
    <div class="bjui-row col-2">
        ...
        <div class="row-input required">
            <input type="text" name="bookinginqty" value="{{$bookinginqty}}" @if($mode=='sure') readonly @endif data-rule="required;number;range[0~]" style="width: 80%">
            <span>（吨）</span>
        </div>
        ...
        <div class="row-input required">
            <input type="text" name="bookingindate" value="{{$bookingindate}}" @if($mode=='sure') readonly @endif data-rule="required">
        </div>
...
<script type="text/javascript">
    function saveBocarDetail() {
        ...
        $('#bookberthin-detail-form').isValid(function(v){
            if (v) {
                var data  = $("#bookberthin-detail-form").serializeJson();
                ...
                if (handlestatus == 'add') {
                    ...
                    $.CurrentNavtab.find('#bookberthin-detail-table').datagrid('updateRow', "{{$gridIndex}}" , data);
                    var obj = $.CurrentNavtab.find('#bookberthin-detail-table').data('allData');
                    obj["{{$gridIndex}}"] = data;
                    $.CurrentNavtab.find('#bookberthin-detail-table').datagrid('reload',  {data:obj});
                    BJUI.dialog('closeCurrent','');
                }
            }else{
                console.log('no');
