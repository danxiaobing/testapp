    ...
    <div class="bs-example">
        <form action="{{$action}}" id="stockout-receipe-form"  class="datagrid-edit-form" data-toggle="validate" data-data-type="json " data-validator-option="{stopOnError:false,timely:false}">
            ...
            <div class="bjui-row col-1">
                    ...
                    <input type="text" name="stockno" value="{{$stockno}}" readonly data-rule="required" data-toggle="findgridbtn" data-options="{
                        ...
                        gridOptions: {
                            ...
                            columns: [
                                ...
                                {name:'instockdate', label:'入库日期',width:120},
                                {name:'goodsname', label:'品名',width:100},
                                {name:'goodsqualityname', label:'质量标准',width:100},
                ...
                <div class="row-input required">
                    <input type="text" name="tobeqty" value="" data-rule="required">
                </div>


                <label class="row-label">实提数量</label>
                <div class="row-input required">
                    <input type="text" name="beqty" value="" data-rule="required">
                </div>
                ...
                @elseif($stockouttype == 2)
                    ...
                    <div class="row-input ">
                        <input type="text" name="fullcarqty" value=""  >
                    </div>
                    ...
                    <div class="row-input ">
                        <input type="text" name="emptycarqty" value=""  >
                    </div>
                @endif
...
<script type="text/javascript">
    function subOutReceipe() {
        ...
        $('#stockout-receipe-form').isValid(function(v){
            if (v) {
                var data  = $("#stockout-receipe-form").serializeArray();

                var json = serializeObject(data);


                var allData  = $("#stockout-receipt-table").data('allData');
            //    console.log(allData);
         ...
         //       console.log(allData);
                ...
                $('#stockout-receipt-table').datagrid('reload',  {data:allData});
