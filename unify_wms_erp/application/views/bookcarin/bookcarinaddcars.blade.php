...
<script type="text/javascript">
    function saveBocars() {
        ...
        $('#bookcarin-cars-form').isValid(function(v){
            if (v) {
                var carid = $.CurrentDialog.find("#bookcarincarid").val();
                BJUI.ajax('doajax', {
                    ...
                    okCallback: function(json, options) {
                        if(json == 1){
                            ...
                            if (handlestatus == 'edit') {
                                $.CurrentNavtab.find('#bookcarin-cars-table').datagrid('updateRow', "{{$gridIndex}}" , data);
                                var obj = $.CurrentNavtab.find('#bookcarin-cars-table').data('allData');
