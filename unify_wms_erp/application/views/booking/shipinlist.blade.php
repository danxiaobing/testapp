...
<div class="bjui-pageContent clearfix">
    <table class="table table-bordered" id="booking{{$navid}}list-table" data-toggle="datagrid" data-options="{
        ...
        showToolbar: true,
        toolbarItem: '',
        toolbarCustom:'#{{$navid}}custom_ship_tb',
        dataUrl: '{{$dataurl}}',
        dataType: 'json',
        jsonPrefix: 'obj',
        editMode: false,
...
<div id="{{$navid}}custom_ship_tb">
    <button type="button" class="btn btn-red" data-icon="reply" id="generate{{$navid}}_shipbackid_btn">船入库退回</button>
    <!-- <button type="button" class="btn btn-green" data-icon="edit" id="login_book_shipin">登记放行信息</button> -->
    <button type="button" class="btn btn-blue" data-icon="plus" id="generate{{$navid}}_btn">生成船入库单</button>
</div>
...
<script type="text/javascript">
    ...
    $('#generate{{$navid}}_shipbackid_btn').click(function () {
        var chks = $.CurrentNavtab.find("#booking{{$navid}}list-table");
        if (chks.length < 1) {
            BJUI.alertmsg('warn', '未选中任何行', {displayPosition: 'middlecenter', displayMode: 'fade'});
            return;
        ...
        if (!checkdata) {
            ...
            BJUI.navtab({
                id: 'backtab335',
                url: '/bookshipin/edit/type/back/booking_sysno/' + checkdata[0].sysno,
                title: '船入库退回',
                mask: true,
            });
        } else if (checkdata.length > 1) {
