<div class="bjui-pageContent">
    <form id="bookcarin-cars-form" action="" class="datagrid-edit-form" data-toggle="validate" data-data-type="json">
        <div class="bjui-row col-2">

            <label class="row-label">车牌</label>
            <div class="row-input required">
                <input type="text" name="carid" id="bookcarincarid" value="{{$carid}}" data-rule="required" data-toggle="findgrid" data-options="{
...
<script type="text/javascript">
    function saveBocars() {
        ...
        $('#bookcarback-cars-form').isValid(function(v){
            if (v) {
                var carid = $("#bookcarincarid").val();
                BJUI.ajax('doajax', {
                    url: 'supplier/checkcarid/carid/'+carid,
                    loadingmask: true,
                    okCallback: function(json, options) {
                        if(json == 1){
                            var data  = $("#bookcarin-cars-form").serializeJson();
                            var allData  = $("#bookcarback-cars-table").data('allData');
                            if (handlestatus == 'add') {
                                ...
                                $('#bookcarin-cars-table').datagrid('reload',  {data:allData});
                                BJUI.dialog('closeCurrent');
                            }
                            if (handlestatus == 'edit') {
                                $.CurrentNavtab.find('#bookcarback-cars-table').datagrid('updateRow', "{{$selectedDatas['gridIndex']}}" , data);
                                var obj = $.CurrentNavtab.find('#bookcarback-cars-table').data('allData');
