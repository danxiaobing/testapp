    ...
    <div class="app-listbox">
        ...
        <div class="app-one">
            <ul>
                ...
                <li><span>已入库数量:</span><span><em>{{$beqty}}</em>吨</span></li>
                <li><span>待入库数量:</span><span><em>{{$daiti_beqty}}</em>吨</span></li>
            </ul>
        </div>
        <h5>车辆信息[{{$count}}]</h5>
        <div class="app-list">
            @foreach($carList as $value)
            <a href="/webapi/carInPoundDetail/sysno/{{$value['sysno']}}">
                <dl>
                    <dt><span>{{$value['carid']}}</span><span class="text-center"><i>已入库</i></span><span class="text-right">{{$value['beqty']}} <i>吨</i></span></dt>
