<div class="bjui-pageHeader">
    <form data-toggle="ajaxsearch" action="{{$action}}" data-options="{searchDatagrid:$.CurrentNavtab.find('#quality_skip_table')}">
        <fieldset>
...
<div class="bjui-pageContent clearfix">
    <table class="table table-bordered" data-toggle="datagrid" id="quality_skip_table" data-options="{
        height: '100%',
        showToolbar: true,
        toolbarCustom:'#quality_skip_bar',
        dataUrl: 'qualitycheck/auditListJson/orderstatus/6',
        dataType: 'json',
        jsonPrefix: 'obj',
        paging: {pageSize:11},
        ...
        <thead>
            <tr data-options="{name:'sysno'}">
                ...
                <th data-options="{name:'orderstatus',align:'center',render:function(value){if(value==6){return '让步待审核';}} }">单据状态</th>
            </tr>
        </thead>
    </table>
</div>
<div id="quality_skip_bar">
    <button type="button" class="btn btn-green" data-icon="gavel"  id="quality_skip_btn">审核</button>
</div>

<script>
    $('#quality_skip_btn').click(function(){
        var checkdata=$('#quality_skip_table').data('selectedDatas');
        if(checkdata && checkdata.length>0){
            ...
            BJUI.navtab({
                id:'qualitycheck-skip',
                mask:true,
                type:'POST',
                url:'/Qualitycheck/qualitycheckedit/tempType/skip/'+id,
                data:{sysno:id},
                title:'品检审核编辑'
            });
        }else{
