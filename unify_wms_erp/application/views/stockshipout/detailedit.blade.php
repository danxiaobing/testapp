...
<div class="bjui-pageContent">
    <div class="bs-example">
        <form action="{{$action}}" id="ship-stockout-receipe-form"  class="datagrid-edit-form" data-toggle="validate" data-data-type="json " data-validator-option="{stopOnError:false,timely:false}">
            <input type="hidden" name="id" value="{{$id}}">
            <div class="bjui-row col-2">

                <input type="hidden" name="sysno" value="{{$sysno}}">
                <input type="hidden" name="stock_sysno" value="{{$stock_sysno}}">
                <input type="hidden" id='ship_goods_sysno' name="goods_sysno" value="{{$goods_sysno}}">
                ...
                <!-- 储罐货品ID -->
                <input type="hidden" id="ship_storagetankgoods_sysno" name="storagetankgoods_sysno" value="{{$goods_sysno}}">
                <!-- 储罐可用余量 -->
                ...
                <div class="row-input">
                <input type="text" name="shipname" readonly value="{{$shipname or ''}}" data-toggle="findgrid" data-options="{
...
<script type="text/javascript">
    function subOutReceipe() {
        ...
        var goods_sysno = $("#ship_goods_sysno").val();
        var storagetankgoods_sysno = $("#ship_storagetankgoods_sysno").val();
        ...
        $('#ship-stockout-receipe-form').isValid(function(v){
            if (v) {
                var data  =  $("#ship-stockout-receipe-form").serializeJson();
                if(data.bookout_detail_sysno =='')
                        data.bookout_detail_sysno = data.sysno;
                $.CurrentNavtab.find('#ship-stockout-receipt-table').datagrid('updateRow', "{{$gridIndex}}" , data);
