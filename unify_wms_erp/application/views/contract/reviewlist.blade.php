...
<div class="bjui-pageContent clearfix">
    <table class="table table-bordered" id="reviewlist-table" data-toggle="datagrid" data-options="{
    	height:'100%',
        showToolbar: true,
        toolbarCustom:'#review_btn',
        dataUrl: '/contract/reviewJson',
        dataType: 'json',
        jsonPrefix: 'obj',
        editMode:false,
        paging: {pageSize:12},
        ...
        <thead>
            <tr data-options="{name:'sysno'}">
                ...
                <!-- <th data-options="{align:'center',render:function(value){return '<button data-icon=edit class=btn-red onclik=review(sysno)>审核</button>'}}">操作</th> -->
            </tr>
        </thead>
    </table>
</div>
<div id="review_btn">
    <button type="button" class="btn btn-blue" data-icon="chain-broken" id="review_btn">评审</button>
</div>
<script type="text/javascript">
$('#review_btn').click(function(){
	var checkdata=$('#reviewlist-table').data('selectedDatas');
	if(checkdata && checkdata.length>0){
		BJUI.dialog({
			width:'1024',
			height:'768',
		    id:'review_btn',
		    url:'/contract/reviewedit/id/'+checkdata[0].sysno,
		    title:'合同评审',
            mask:true,
		});
	}else{
		BJUI.alertmsg('warn','<h4>未选中行</h4>');
	}
