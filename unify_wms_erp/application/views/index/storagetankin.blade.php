            ...
            <ul class="bar clearfix">
                @if(isset($sdata['storageData']))
                    @foreach($sdata['storageData'] as $value)
                        <li>
                            <div class="bottom" >
                                <div class="blockDt2 active" id="example2_{{$value['sysno']}}">
                                    <p>{{$value['actualcapacity']}}T</p>
                                    <p>{{$value['tankinqty']}}T</p>
                                    <p class="theoreticalcapacity">{{round(($value['tankinqty'])/$value['actualcapacity']*100).'%'}}</p>
                                </div>
                            </div>
                            <div class="top"></div>
...
$(function(){
    $(".blockDt2").each(function(index, el) {
