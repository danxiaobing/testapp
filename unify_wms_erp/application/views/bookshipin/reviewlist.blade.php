<div class="bjui-pageHeader " style="background-color:#fefefe; border-bottom:none;">
    <form data-toggle="ajaxsearch" data-options="{searchDatagrid:$.CurrentNavtab.find('#bookshipinreviewlist-table')}">
        <fieldset>
...
<div class="bjui-pageContent clearfix">
    ...
    <table class="table table-bordered" id="bookshipinreviewlist-table" height="40+50*{{count($list)}}"
           data-toggle="datagrid" data-options="{
           tableWidth:'1600',
        height: '100%',
        showToolbar: true,
        toolbarCustom: '#review_bookshipin',
        addLocation: 'last',
        dataUrl: 'bookshipin/reviewJson',
        dataType: 'json',
        jsonPrefix: 'obj',
        ...
        <tr data-options="{name:'sysno'}">
            <th data-options="{name:'bookinginno',align:'center'}">入库预约单号</th>
...
<div id="review_bookshipin">
    <button type="button" id="review_bookshipin_btn" class="btn btn-green" data-icon="gavel">审核</button>
</div>

<script>
    $("#review_bookshipin_btn").click(function () {
        //var chks = $.CurrentNavtab.find("input:checked");
        var chks  = $("#bookshipinreviewlist-table").data('selectedDatas');
        if (chks.length < 1) {
            BJUI.alertmsg('warn', '未选中任何行', {displayPosition: 'middlecenter', displayMode: 'fade'});
            return;
        }
        var checkdata = $('#bookshipinreviewlist-table').data('selectedDatas');
        ...
        BJUI.navtab({
            id: 'reviewshipin980',
            mask: true,
            url: '/bookshipin/edit/type/review/booking_sysno/' + checkdata[0].sysno,
            title: '审核船入库预约单'
        });
    });
</script>